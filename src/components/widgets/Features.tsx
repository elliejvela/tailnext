...
const Features = ({ id, header, items, columns = 3, hasBackground = false }: FeaturesProps) => (
  <WidgetWrapper id={id ? id : ''} hasBackground={hasBackground} containerClass="scroll-mt-16 max-w-6xl">
    {header && <Headline header={header} titleClass="text-4xl md:text-5xl text-[#1F0024]"/>}
    <ItemGrid
      id={id}
      items={items}
      columns={columns}
      ...
      iconClass="h-12 w-12 flex items-center justify-center rounded-md text-white bg-[#693371] p-2 md:p-3 mt-1.5 mb-4 mr-4 rtl:ml-4 rtl:mr-0"
      titleClass="mb-3 text-xl font-bold text-[#1F0024]"
